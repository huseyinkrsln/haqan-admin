...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0d0b] text-zinc-100 flex items-center justify-center p-4 sm:p-6 relative overflow-hidden selection:bg-[#4A5D3E] selection:text-white">
      ...
      {/* ─── MERKEZİ LÜKS KAYIT KARTI (Obsidian Glassmorphism) ─── */}
      <div className="w-full max-w-md bg-zinc-900/85 backdrop-blur-2xl border border-zinc-800/90 rounded-3xl p-8 sm:p-11 shadow-[0_25px_80px_rgba(0,0,0,0.8)] relative z-10 animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="text-center space-y-3 mb-8">
          ...
          <div>
            ...
            <span className="text-[10px] uppercase tracking-[0.35em] text-[#a3b899] font-semibold block mt-1">
              YÖNETİCİ HESABI OLUŞTUR
            </span>
          </div>

          <p className="text-xs text-zinc-400 font-light">
            Yönetim ekibine katılmak için bilgilerinizi eksiksiz doldurun
          </p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div className="space-y-3.5">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-zinc-300 uppercase tracking-wider block" htmlFor="name">
                Ad Soyad
              </label>
          ...
          <Button
            type="submit"
            disabled={isLoading}
            className="w-full h-12 text-xs font-bold uppercase tracking-[0.2em] bg-[#4A5D3E] hover:bg-[#3D4D33] text-white shadow-lg shadow-[#4A5D3E]/20 transition-all hover:-translate-y-0.5 rounded-xl cursor-pointer mt-2"
          >
            {isLoading ? (
        ...
        <div className="pt-6 border-t border-zinc-800/80 space-y-3 text-center">
          <div className="flex items-center justify-center gap-2 text-[11px] text-zinc-400">
            ...
            <span>Yetkili Yönetici Erişim Güvenliği</span>
          </div>

          <p className="text-xs text-zinc-500">
            Zaten bir hesabınız var mı?{" "}
            <Link href="/login" className="text-[#a3b899] hover:underline font-semibold">
              Giriş Yapın
            </Link>
          </p>
        </div>
